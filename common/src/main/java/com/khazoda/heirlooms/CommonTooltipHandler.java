...
public class CommonTooltipHandler {
  ...
  public static void handleTooltip(ItemStack stack, List<Component> tooltip, TooltipFlag flag, boolean isCtrlPressed, boolean isAltPressed) {
    ...
    String craftedTimestamp = null;
    String craftedBy = null;
    String enchantedTimestamp = null;
    String enchantedBy = null;

    if (tag != null) {
      if (tag.contains(HeirloomsCommon.CRAFTED_TIMESTAMP))
        craftedTimestamp = tag.getString(HeirloomsCommon.CRAFTED_TIMESTAMP);
      if (tag.contains(HeirloomsCommon.CRAFTED_BY)) craftedBy = tag.getString(HeirloomsCommon.CRAFTED_BY);
      if (tag.contains(HeirloomsCommon.ENCHANTED_TIMESTAMP))
        enchantedTimestamp = tag.getString(HeirloomsCommon.ENCHANTED_TIMESTAMP);
      if (tag.contains(HeirloomsCommon.ENCHANTED_BY)) enchantedBy = tag.getString(HeirloomsCommon.ENCHANTED_BY);
    }

    // Only apply to items with at least one Heirlooms NBT tag
  ...
  private static void addButtonPrompts(List<Component> tooltip, boolean hasCraftData, boolean hasEnchantData, boolean isCtrlPressed, boolean isAltPressed) {
    ...
    if (hasCraftData && !isCtrlPressed)
      buttonPrompt.append(Component.literal("[ᴄᴛʀʟ]").withStyle(style -> style.withColor(CRAFTED_TEXT_COLOR)));
    ...
    if (hasEnchantData && !isAltPressed)
      buttonPrompt.append(Component.literal("[ᴀʟᴛ]").withStyle(style -> style.withColor(ENCHANTED_TEXT_COLOR)));
  ...
  private static void addTimestampTooltip(List<Component> tooltip, String timestamp, String creator, String translationKey, int color) {
    try {
      ...
      tooltip.add(Component.translatable(translationKey, creator).withStyle(style -> style.withColor(color)));
